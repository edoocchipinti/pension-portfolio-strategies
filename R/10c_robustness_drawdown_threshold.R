# 10c_robustness_drawdown_threshold.R
#
# Input:  same as 10b
#
# Output: Data/robustness_C_drawdown_threshold.rds
#
# Sensitivity of ML strategies to the drawdown override threshold.
# Baseline: dd_thr = -15% (Faber 2007, Estrada 2008 conventional bear-market).
# Alternatives: -10% (more sensitive trigger) and -20% (more conservative).
#
# For each alternative, re-label regimes, retrain Lasso/Ridge/RF, apply to
# DNB, compute RR.
...
# ---- 2. Helper functions (same structure as 10b) -------------------------
relabel_regimes <- function(df, slope_thr, infl_thr, dd_thr) {
  df %>%
    mutate(
      regime = case_when(
...
# ---- 3. Invariant strategy RR (computed once) ---------------------------
...
# ---- 4. Loop over drawdown thresholds ------------------------------------
dd_thrs <- c(-0.10, -0.15, -0.20)
labels  <- c("DD_10pct", "DD_15pct_baseline", "DD_20pct")
all_results <- list()

for (i in seq_along(dd_thrs)) {
  dd <- dd_thrs[i]
  lbl <- labels[i]
  cat(sprintf("\n=== Drawdown threshold = %.0f%% (%s) ===\n", 100 * dd, lbl))
  
  df_lab <- relabel_regimes(hist, SLOPE_THR, INFL_THR, dd)
  ...
  for (s_name in names(RR_all)) {
    ...
    all_results[[paste(lbl, s_name, sep = "_")]] <- data.frame(
      strategy = s_name, dd_threshold = lbl,
      dd_threshold_val = dd,
...
cat("\n--- MEDIAN RR BY STRATEGY x DD_THRESHOLD ---\n")
pivot_median <- master %>%
  select(strategy, dd_threshold, RR_median) %>%
  pivot_wider(names_from = dd_threshold, values_from = RR_median) %>%
  mutate(strategy = factor(strategy, levels = strategy_order)) %>%
  arrange(strategy)
print(pivot_median)

cat("\n--- SHORTFALL_70 BY STRATEGY x DD_THRESHOLD ---\n")
pivot_sf <- master %>%
  select(strategy, dd_threshold, P_shortfall_70) %>%
  pivot_wider(names_from = dd_threshold, values_from = P_shortfall_70) %>%
  mutate(strategy = factor(strategy, levels = strategy_order)) %>%
  arrange(strategy)
print(pivot_sf)

# ---- 6. Save -------------------------------------------------------------
saveRDS(master, "Data/robustness_C_drawdown_threshold.rds")
cat("\nSaved Data/robustness_C_drawdown_threshold.rds\n")
