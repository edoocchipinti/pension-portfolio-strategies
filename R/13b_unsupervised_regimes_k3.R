# 13b_unsupervised_regimes_k3.R
#
# HMM regime detection with K = 3 states.
#
# Rationale for switching from K=4 to K=3:
#   V1 (K=4) suffered from non-convergent EM (log-likelihood decreasing in
#   late iterations) and OOD problems on DNB scenarios. BIC diagnostic also
#   suggested k=3 is more stable than k=4 (k=4 jumps suspiciously from
#   2315 to -4595, indicating numerical issues; the most stable interior
#   solution is k=3 with BIC=2315).
#
# Methodological justification (PRE-DECLARED in thesis):
#   The transition from k=4 to k=3 is motivated by BIC stability, NOT by
#   ex-post performance comparison. K=4 was tested first for direct V3
#   comparability; its convergence failures motivate the k=3 alternative.
#   We report both as part of the robustness discussion.
#
# Setup:
#   - K = 3 states
#   - 3-dim Gaussian emissions (slope, inflation, real_yield)
#     [eq_drawdown_12m dropped: high zero-mass causes numerical issues]
#   - Allocation map: 3 regimes (Normal / Inflationary / Risk-off)
#       - Normal:       70% equity  (high slope, low/moderate inflation)
#       - Inflationary: 45% equity  (high inflation regardless of slope)
#       - Risk-off:     30% equity  (low slope, low inflation -> recession risk)
#   - Hard mapping (Viterbi) + Soft mapping (forward-backward)
#
...
# Strip previous HMM rows (idempotency for both k=4 and k=3 attempts)
for (s in c("HMM_hard", "HMM_soft")) {
  if (s %in% metrics_old$strategy) {
    cat(sprintf("Removing previous %s rows\n", s))
...
# DROPPED: eq_drawdown_12m (high zero-mass causes EM issues)
clust_features <- c("slope_10_1", "hicp_yoy", "real_yield10")

hist <- hist[order(hist$date), ]
hist_c <- hist[, c("date", clust_features), drop = FALSE]
n_hist <- nrow(hist_c)

cat("Clustering features (3-feature subset, raw scale):\n")
...
fit_hmm_robust <- function(k, data, features, seeds = c(42, 7, 123, 2024, 999, 555),
  ...
  for (sd_val in seeds) {
    ...
    if (!is.null(fit_obj)) {
      ...
      if (is.finite(ll) && ll > best_ll &&
          !grepl("decreased", fit_obj@message, ignore.case = TRUE)) {
        best_fit <- fit_obj
        best_ll  <- ll
      }
    }
  ...
  if (is.null(best_fit)) {
    cat(sprintf("    All seeds failed for k = %d (or all had non-monotonic EM)\n", k))
  } else {
    cat(sprintf("    Best logLik across seeds (converged only): %.2f\n", best_ll))
  }
  best_fit
}

# ---- 4. BIC diagnostic for k = 2..6 (with 3-feature subset) ------------
cat("Running BIC diagnostic for k = 2..6 with 3-feature subset...\n")
...
cat("\n--- BIC DIAGNOSTIC (k=2..6, 3 features, converged fits only) ---\n")
print(bic_results)

# ---- 5. Main fit with K = 3 ---------------------------------------------
K <- 3
cat(sprintf("\nFitting main HMM with k = %d on %d obs (3 standardized features)...\n",
            K, n_hist))
fit_main <- fit_hmm_robust(K, X_clust, clust_features)
if (is.null(fit_main)) {
  stop("Main HMM k=3 fit failed. Try declaring HMM as limitation.")
}
...
# ---- 7. Map 3 states to economic regimes ------------------------------
# Logic:
#   - State with HIGHEST inflation                -> Inflationary (45%)
#   - Among remaining 2: HIGHER slope             -> Normal (70%)
#                        LOWER slope              -> Risk-off (30%)
...
# Step 1: identify highest-inflation state
infl_state <- which.max(state_score$infl_mean)
remaining_states <- setdiff(1:K, infl_state)

# Step 2: among remaining, higher slope = Normal, lower slope = Risk-off
slope_remaining <- state_score$slope_mean[remaining_states]
normal_state    <- remaining_states[which.max(slope_remaining)]
riskoff_state   <- remaining_states[which.min(slope_remaining)]

state_to_regime <- character(K)
state_to_regime[infl_state]    <- "Inflationary"
state_to_regime[normal_state]  <- "Normal"
state_to_regime[riskoff_state] <- "Risk_off"
names(state_to_regime) <- 1:K

cat("\n--- STATE TO REGIME MAPPING ---\n")
for (s in 1:K) {
  cat(sprintf("  State %d (slope=%.2f, infl=%.2f): %s\n",
              s, state_score$slope_mean[s], state_score$infl_mean[s],
              state_to_regime[s]))
}

# 3-regime allocation map
alloc_map_k3 <- c(Normal = 0.70, Inflationary = 0.45, Risk_off = 0.30)
state_to_alloc <- alloc_map_k3[state_to_regime]
names(state_to_alloc) <- 1:K

cat("\n--- STATE TO EQUITY WEIGHT MAPPING ---\n")
print(state_to_alloc)

# ---- 8. Historical Viterbi + posteriors -------------------------------
...
# Compare hard vs soft diff
cat(sprintf("Mean |Hard - Soft|: %.4f\n",
            mean(abs(hist_alloc_hard - hist_alloc_soft))))
cat("(If ~0, soft adds no smoothing; if >0.05, soft is meaningfully different.)\n\n")

# ---- 9. HMM vs V3 historical regimes (for narrative) ------------------
cat("--- HMM (k=3) vs V3 (k=4) REGIME COMPARISON ---\n")
...
hmm_regimes <- factor(state_to_regime[as.character(vit_states)],
                      levels = c("Normal", "Inflationary", "Risk_off"))

cat("V3 distribution (4-regime taxonomy):\n")
print(table(v3_regimes))
cat("\nHMM_k3 distribution (3-regime taxonomy):\n")
print(table(hmm_regimes))

cat("\nCross-tabulation (rows = V3, cols = HMM_k3):\n")
print(table(V3 = v3_regimes, HMM_k3 = hmm_regimes))

# Allocation correlation: convert both to equity weights, check correlation
v3_alloc_map <- c(Normal_expansion = 0.70, Inflationary_exp = 0.55,
                  Late_cycle = 0.45, Stagflation_risk = 0.30)
v3_alloc_hist <- v3_alloc_map[as.character(v3_regimes)]
alloc_corr <- cor(v3_alloc_hist, hist_alloc_hard)
cat(sprintf("\nCorrelation V3 equity allocation vs HMM_k3 hard: %.3f\n", alloc_corr))

# ---- 10. DNB Viterbi+FB application ----------------------------------
cat("\nBuilding DNB feature trajectories...\n")
dnb_features <- array(NA_real_, dim = c(N_SCEN, N_YEARS, 3))
...
cat("DNB feature summary at year 5 (sanity):\n")
...
cat(sprintf("HMM_soft (k=3):\n"))
cat(sprintf("  mean=%.3f, sd=%.3f, range=[%.2f, %.2f]\n\n",
            mean(soft_alloc_dnb), sd(soft_alloc_dnb),
            min(soft_alloc_dnb), max(soft_alloc_dnb)))

cat("HMM_hard DNB state distribution:\n")
state_dnb_dist <- prop.table(table(factor(hard_states_dnb, levels = 1:K))) * 100
names(state_dnb_dist) <- paste0(names(state_dnb_dist), " (", state_to_regime[names(state_dnb_dist)], ")")
print(round(state_dnb_dist, 1))
...
cat("\nComputing wealth, MDD, turnover, RR...\n")
...
cat(sprintf("HMM_hard (k=3):  RR_median=%.3f, shortfall_70=%.3f, CVaR_5=%.3f, MDD_mean=%.3f\n",
            median(RR_hard), mean(RR_hard < 0.70),
            mean(RR_hard[RR_hard <= quantile(RR_hard, 0.05)]),
            mean(mdd_hard)))
cat(sprintf("HMM_soft (k=3):  RR_median=%.3f, shortfall_70=%.3f, CVaR_5=%.3f, MDD_mean=%.3f\n\n",
...
saveRDS(list(
  fit = fit_main,
  K_selected = K,
  K_justification = "BIC stability + EM convergence (k=4 failed)",
  features_used = clust_features,
...
cat("\nSaved (HMM k=3, 3-feature subset):\n")
cat("  Data/hmm_model.rds\n")
cat("  Data/hmm_state_assignments.rds\n")
cat("  Data/hmm_dnb_allocations.rds\n")
